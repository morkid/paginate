...
func TestStartsWithStringFilter(t *testing.T) {
	size := 20
	page := 1
	sort := "user.name,-id"
	avg := "seventy"
...
func TestEndsWithStringFilter(t *testing.T) {
	size := 20
	page := 1
	sort := "user.name,-id"
	avg := "seventy"
	...
	queryFilter := fmt.Sprintf(`[["username","endswith","%s"]]`, avg)
	query := fmt.Sprintf(data, page, size, sort, queryFilter)
...
func TestExactStringFilter(t *testing.T) {
	...
	queryFilter := fmt.Sprintf(`[["username","elike","%s"]]`, avg)
	query := fmt.Sprintf(data, page, size, sort, queryFilter)
